# Documentação do projeto

Liste os documentos disponibilizados nesta pasta com uma breve descrição do conteúdo de cada um deles.

* relatorio_final.pdf - Relatório do projeto.

Relatório completo do projeto, contendo a descrição dos objetivos, desenvolvimento e resultados obtidos.

[projetoeixo2.pdf](https://github.com/user-attachments/files/18055096/projetoeixo2.pdf)

* apresentacao.mkv - Vídeo de apresentação do projeto.

Vídeo de apresentação do projeto, demonstrando as principais funcionalidades e explicações detalhadas.

https://github.com/user-attachments/assets/ae09b813-57a5-4a42-9386-d9ba888c388e

* slides.pptx - Slides da apresentação final do projeto.

Arquivo de slides utilizado na apresentação final, com destaque para o objetivo, solução implementada e resultados.

[ApresentaçãoSmartfleet.pptx](https://github.com/user-attachments/files/18055112/ApresentacaoSmartfleet.pptx)

